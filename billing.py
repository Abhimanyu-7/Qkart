...
def post(label,price,final_rate,taken):
    ...
    url = "https://peepss-production.up.railway.app/product"
...
def list_com(label,final_weight):
    global count, taken, list_label, list_weight
    if final_weight > 2 :	
...
def rate(final_weight, label, taken):
    ...
    elif label == c:   # Cinthol
        print("Calculating rate of", label)
        final_rate = final_weight * 0.02
        price = 20
        post(label, price, final_rate, taken)

    elif label == h:   # HideNSeek
        print("Calculating rate of", label)
        final_rate = 1
        price = 1
        post(label, price, final_rate, taken)
                ...
                if "classification" in res["result"].keys():
                    ...
                    for label in labels:
                        ...
                        if score > 0.85 :
                            ...
                            if label == m:
                                print('Maggi detected')       
                            elif label == c:
                                print('Cinthol detected')
                            elif label == mp:
                                print('Milk Powder deteccted')
                            elif label == h :
                                print('HideNSeek detected')
                            elif label == f:
                                print('Fanta detected')
                            elif label == t:
                                print('Takatak detected')
                            else:
                                print('Popcorn detected')
                    print('', flush=True)
                next_frame = now() + 100
        finally:
            if (runner):
